...
import { BaseDiskoperinEntity, DISKOPERIN_OPD_ID } from "./types";
import {
  baseColumns,
  createActionsColumn,
  createNumericColumn,
...
interface CrudPageProps<T extends BaseDiskoperinEntity> {
  title: string;
  description: string;
  apiEndpoint: string;
  queryKey: string;
  columns: ColumnConfig[];
  defaultData: Omit<T, keyof BaseDiskoperinEntity>;
...
export function CrudPage<T extends BaseDiskoperinEntity>({
  title,
  description,
  apiEndpoint,
  queryKey,
  ...
  const { data } = useQuery({
    queryKey: [queryKey],
    queryFn: () =>
      fetch(`/api/${apiEndpoint}?id_opd=${DISKOPERIN_OPD_ID}`).then((res) =>
        res.json()
      ),
  });
