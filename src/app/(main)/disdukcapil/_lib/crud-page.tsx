...
import { BaseDisdukcapilEntity, DISDUKCAPIL_OPD_ID } from "./types";
import {
  baseColumns,
  createActionsColumn,
  createNumericColumn,
...
interface CrudPageProps<T extends BaseDisdukcapilEntity> {
  title: string;
  description: string;
  apiEndpoint: string;
  queryKey: string;
  columns: ColumnConfig[];
  defaultData: Omit<T, keyof BaseDisdukcapilEntity>;
...
export function CrudPage<T extends BaseDisdukcapilEntity>({
  title,
  description,
  apiEndpoint,
  queryKey,
  ...
  const { data } = useQuery({
    queryKey: [queryKey],
    queryFn: () =>
      fetch(`/api/${apiEndpoint}?id_opd=${DISDUKCAPIL_OPD_ID}`).then((res) =>
        res.json()
      ),
  });
