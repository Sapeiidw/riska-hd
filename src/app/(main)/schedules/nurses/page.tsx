...
import { NurseScheduleForm } from "./nurse-schedule-form";
...
const STATUS_MAP: Record<
  string,
  { label: string; variant: "default" | "success" | "destructive" | "secondary" | "outline" }
> = {
  scheduled: { label: "Terjadwal", variant: "outline" },
  present: { label: "Hadir", variant: "success" },
  absent: { label: "Tidak Hadir", variant: "destructive" },
  leave: { label: "Cuti", variant: "secondary" },
};

async function fetchSchedules(startDate?: string, endDate?: string, shiftId?: string) {
  const params = new URLSearchParams({ limit: "500" });
  if (startDate) params.set("startDate", startDate);
  if (endDate) params.set("endDate", endDate);
  if (shiftId) params.set("shiftId", shiftId);

  const { data } = await api.get(`/api/schedules/nurses?${params}`);
  return data;
}
...
async function deleteSchedule(id: string) {
  const { data } = await api.delete(`/api/schedules/nurses/${id}`);
  return data;
}

async function updateScheduleDate(id: string, newDate: string) {
  const { data } = await api.put(`/api/schedules/nurses/${id}`, { scheduleDate: newDate });
  return data;
}
...
export default function NurseSchedulesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["nurse-schedules", startDate, endDate, shiftFilter],
    queryFn: () => fetchSchedules(startDate, endDate, shiftFilter || undefined),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteSchedule,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["nurse-schedules"] });
  ...
  const calendarEvents: ScheduleEvent[] = useMemo(() => {
    ...
    return data.data.map((schedule: NurseSchedule) => {
      ...
      return {
        id: schedule.id,
        title: `${schedule.nurseName} - ${schedule.shiftName}`,
        start: startDateTime,
        end: endDateTime,
        extendedProps: {
  ...
  const handleEventClick = (eventId: string, event: ScheduleEvent) => {
    const schedule = data?.data?.find((s: NurseSchedule) => s.id === eventId);
    if (schedule) {
  ...
  const columns: ColumnDef<NurseSchedule>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            {format(new Date(row.getValue("scheduleDate")), "EEE, dd MMM yyyy", {
              locale: localeId,
            })}
          </span>
        </div>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-medium">{row.getValue("nurseName")}</span>
          {row.original.nurseNip && (
            <span className="text-xs text-muted-foreground">NIP: {row.original.nurseNip}</span>
          )}
        </div>
      ),
    },
      ...
      cell: ({ row }) => {
        ...
        return roomName ? (
          <Badge variant="outline">
            {roomName} ({row.original.roomCode})
          </Badge>
        ) : (
  ...
  const presentCount = data?.data?.filter((s: NurseSchedule) => s.status === "present").length || 0;
  const absentCount = data?.data?.filter((s: NurseSchedule) => s.status === "absent").length || 0;
  ...
  const stats = data?.meta
    ? [
        ...
        {
          label: "Terjadwal",
          value: data.data?.filter((s: NurseSchedule) => s.status === "scheduled").length || 0,
          icon: CalendarCheck,
          color: "default" as const,
        },
        {
  ...
  return (
    <>
      <MasterPageLayout
        ...
        description="Kelola jadwal kerja perawat per shift - Drag & drop untuk memindahkan jadwal"
        icon={Users}
        stats={stats}
      ...
      >
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Terjadi kesalahan saat memuat jadwal perawat"
          />
        ) : viewMode === "calendar" ? (
          <ScheduleCalendar
          ...
          <EmptyState
            ...
            description="Mulai dengan menambahkan jadwal perawat baru"
          />
        ) : (
          <DataTable
            columns={columns}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingSchedule ? "Edit Jadwal Perawat" : "Tambah Jadwal Perawat"}
            </DialogTitle>
          </DialogHeader>
          <NurseScheduleForm
