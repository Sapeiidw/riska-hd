...
import { DISNAKER_OPD_ID } from "./types";
...
export function useKenaikanPangkat(year: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ChartRecord[]>(`/api/kenaikan-pangkat?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}`),
  });
}
...
export function useStatusDokumen(year: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ChartRecord[]>(`/api/status-dokumen-wajib?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}`),
  });
}
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ChartRecord[]>(
        `/api/status-kenaikan-pangkat?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}&month=${month}`
      ),
  });
}
...
export function useStatusSKKenaikanPangkat(year: string, month: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ChartRecord[]>(
        `/api/status-sk-kenaikan-pangkat?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}&month=${month}`
      ),
  });
}
...
export function useGolonganPegawai(year: string, month: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ChartRecord[]>(
        `/api/golongan-pegawai?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}&month=${month}`
      ),
  });
}
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<StatusPegawaiRow[]>(
        `/api/status-pegawai?for=dashboard&id_opd=${DISNAKER_OPD_ID}&year=${year}&month=${month}`
      ),
  });
}
