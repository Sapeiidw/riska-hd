...
import { BaseDpuprEntity, DPUPR_OPD_ID } from "./types";
import {
  baseColumns,
  createActionsColumn,
  createNumericColumn,
...
interface CrudPageProps<T extends BaseDpuprEntity> {
  title: string;
  description: string;
  apiEndpoint: string;
  queryKey: string;
  columns: ColumnConfig[];
  defaultData: Omit<T, keyof BaseDpuprEntity>;
...
export function CrudPage<T extends BaseDpuprEntity>({
  title,
  description,
  apiEndpoint,
  queryKey,
  ...
  const { data } = useQuery({
    queryKey: [queryKey],
    queryFn: () =>
      fetch(`/api/${apiEndpoint}?id_opd=${DPUPR_OPD_ID}`).then((res) =>
        res.json()
      ),
  });
