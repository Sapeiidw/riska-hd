...
import { BaseRsudEntity, RSUD_OPD_ID } from "./types";
import {
  baseColumns,
  createActionsColumn,
  createNumericColumn,
...
interface CrudPageProps<T extends BaseRsudEntity> {
  title: string;
  description: string;
  apiEndpoint: string;
  queryKey: string;
  columns: ColumnConfig[];
  defaultData: Omit<T, keyof BaseRsudEntity>;
...
export function CrudPage<T extends BaseRsudEntity>({
  title,
  description,
  apiEndpoint,
  queryKey,
  ...
  const { data } = useQuery({
    queryKey: [queryKey],
    queryFn: () =>
      fetch(`/api/${apiEndpoint}?id_opd=${RSUD_OPD_ID}`).then((res) =>
        res.json()
      ),
  });
