...
import { BaseDisporaparEntity, DISPORAPAR_OPD_ID } from "./types";
import {
  baseColumns,
  createActionsColumn,
  createNumericColumn,
...
interface CrudPageProps<T extends BaseDisporaparEntity> {
  title: string;
  description: string;
  apiEndpoint: string;
  queryKey: string;
  columns: ColumnConfig[];
  defaultData: Omit<T, keyof BaseDisporaparEntity>;
...
export function CrudPage<T extends BaseDisporaparEntity>({
  title,
  description,
  apiEndpoint,
  queryKey,
  ...
  const { data } = useQuery({
    queryKey: [queryKey],
    queryFn: () =>
      fetch(`/api/${apiEndpoint}?id_opd=${DISPORAPAR_OPD_ID}`).then((res) =>
        res.json()
      ),
  });
